...
[Serializable]
public class EasyPool : IDisposable, IEnumerable
{
    ...
    /// </summary>
    public Func<GameObject> InstantiateFunction { get; set; }
    ...
    public EasyPool(Func<GameObject> instantiateFunction)
    {
        list = new List<GameObject>();
    ...
    public GameObject Get()
    {
        ...
        if (!go && !IsFull) list.Add(go = InstantiateFunction());
        if (go)
        {
            go.SetActive(true);
    ...
    /// <summary>
    /// Invokes <see cref="Return(GameObject)"/> on all objects instantiated by this pool.
    /// </summary>
    ...
    /// <summary>
    /// Invokes <see cref="Return(GameObject)"/> on all objects instantiated by this pool that currently return <see cref="UsableCriteria"/> = false.
    /// </summary>
    public void ReturnAllActive()
    {
        foreach (GameObject go in list)
            if (go && !UsableCriteria(go))
                Return(go);
    }
