...
public class EasyGameSave
{
    /// <summary>
    /// Convenience property to get currently running GameSave
    /// </summary>
    public static EasyGameSave Current => EasyGameLoader.GameSaves != null && EasyGameLoader.GameSaves.TryGetValue(EasyGameLoader.CurrentSaveSlot, out EasyGameSave save) ? save : null;

    public static bool TryGetCurrent(out EasyGameSave current) => (current = Current) != null;

    public EasyGameSave() { }

    public EasyGameSave(string version) => GameVersion = version;

    public EasyGameSave(string version, string sceneName, bool save = true)
    {
        GameVersion = version;
        SceneName = sceneName;
        if (save) EasyGameLoader.SaveGame();
    }
    ...
    public bool IsCompatibleVersion => EasyGameLoader.IsCompatibleSaveVersion(GameVersion);
    ...
    public static void SetVisitedScene(string key, bool value = true, bool save = true)
    {
        ...
        if (changed && save) EasyGameLoader.SaveGame();
    }
    ...
    /// PLEASE USE STATIC VERSIONS INSTEAD
    /// </summary>
    public HashSet<string> CustomCondition = new HashSet<string>();

    public static bool GetCustomCondition(string key) => Current?.CustomCondition?.Contains(key) ?? false;
    ...
    /// <param name="value"></param>
    /// <param name="save"></param>
    public static void SetCustomCondition(string key, bool value = true, bool save = true)
    {
        if (Current == null) return;
        bool changed = value ? Current.CustomCondition.Add(key) : Current.CustomCondition.Remove(key);
        if (changed && save) EasyGameLoader.SaveGame();
    }
